    ...
    if not st.session_state[f"{user_session_id}_round_{round_num}_input_completed"]:
        ...
        with tab1:
            ...
            with st.expander(f"**🤖 AI 回應設定**", expanded=False):  # 預設不展開
                ...
                # 是否要互相給對方Agent的回答
                ai_feedback_enabled = st.checkbox("開啟 AI 互相回饋", disabled=len(selected_agents) < 2, key=f"{user_session_id}_ai_feedback_enabled_{round_num}_free_input")
        ...
        with tab2:
            ...
            with st.expander(f"**🤖 AI 回應設定**", expanded=False):  # 預設不展開
                ...
                # 是否要互相給對方Agent的回答
                ai_feedback_enabled = st.checkbox("開啟 AI 互相回饋", disabled=len(selected_agents) < 2, key=f"{user_session_id}_ai_feedback_enabled_{round_num}_scamper_input")
                if len(selected_agents) < 2:
                    st.info("⚠️ 至少需要選擇兩位 Agent 才能啟用互相回饋功能")
                st.session_state[f"{user_session_id}_ai_feedback_enabled"] = ai_feedback_enabled
